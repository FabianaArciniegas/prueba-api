...
class ProductsService:
    ...
    async def create_product(self, product_input: ProductInput):
        ...
        product_found = await self.products_repository.get_product_by_code(product_input.product_code)
        if product_found:
    ...
    async def update_product(self, product_code: int, update_data: PatchProductInput) -> ProductsModel:
        ...
        product_by_update = await self.products_repository.get_product_by_code(product_code)
        if product_by_update is None:
            raise ValueError("Product not found")
    ...
    async def update_all_product(self, product_code: int, product_data: ProductInput) -> ProductsModel:
        ...
        product = await self.products_repository.get_product_by_code(product_code)
        if product is None:
            raise ValueError("Product not found")
    ...
    async def delete_product(self, product_code: int):
        ...
        product_by_delete = await self.products_repository.get_product_by_code(product_code)
        if product_by_delete is None:
            raise ValueError("Product not found")
